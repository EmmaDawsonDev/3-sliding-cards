...
buttonLeft.addEventListener("click", function() {
  ...
  if (index === -3) {
    ...
    addUser([data[data.length - 1], data[0], data[1]])
    index = data.length -1;
  } else if (index === -2) {
    addUser([data[data.length - 2], data[data.length - 1], data[0]])
    index = data.length - 2;
  }  else {
    addUser([data[index], data[index + 1], data[index + 2]])
  }
